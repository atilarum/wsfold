...
if ! docker build -f scripts/native-bind-e2e.Dockerfile -t "$image" . >/tmp/wsfold-native-bind-e2e-build.log 2>&1; then
  fail "Docker image build failed; see /tmp/wsfold-native-bind-e2e-build.log"
fi

docker_args=(--rm --cap-add SYS_ADMIN --security-opt apparmor=unconfined \
